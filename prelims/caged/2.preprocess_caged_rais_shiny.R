# =============================================================================
# Build caged_rais_curso + ranking table with AWS/S3 integration
# =============================================================================
rm(list = ls())
gc()
options(scipen = 999)
...
# Function for moving average computation
...
# -----------------------------------------------------------------------------
# 1) Load AWS creds and helpers (same framework as your reference)
# -----------------------------------------------------------------------------
dotenv::load_dot_env()
...
update_data_from_s3 <- function(local_path, s3_path, bucket) {
  if (!file.exists(local_path)) {
    ...
    tryCatch({
      ...
      message("✅ Downloaded from S3: ", s3_path, " -> ", local_path)
    }, error = function(e) {
      stop("❌ Failed to download from S3: ", s3_path, " — ", e$message)
    })
  } else {
...
# -----------------------------------------------------------------------------
# 2) Source paths (S3) and local mirrors
# -----------------------------------------------------------------------------
# Excel crosswalks
...
# RAIS-CAGED preprocessed panel used as input
...
# 3) Read inputs
# -----------------------------------------------------------------------------
# Crosswalk area–CBO and Eixo–Área–Curso
...
# RAIS-CAGED object
load(rda_in_local) # expects object named `caged_rais_preenchido` in your original pipeline

if (!exists("caged_rais_preenchido")) {
  stop("❌ Expected object `caged_rais_preenchido` not found in: ", rda_in_local)
}
...
# 4) Truncate outliers
# -----------------------------------------------------------------------------
caged_rais_preenchido %>% 
  mutate( media_sal_adm = pmax(pmin(media_sal_adm, 10000), 1000), 
          media_sal_des = pmax(pmin(media_sal_adm, 10000), 1000) ) -> caged_rais_preenchido
...
caged_rais_preenchido <- group_by(caged_rais_preenchido, ANO, MES, CO_UF, NM_UF, cbo6) %>% 
  summarise( 
    ...
    dif_sal_adm_des = media_sal_adm - media_sal_des, dif_sal_adm_des_pc = (media_sal_adm - media_sal_des) * 100 / media_sal_des, 
...
# -----------------------------------------------------------------------------
# 6) Map to Cursos (unique Area–Curso–CBO relation) and aggregate by Curso
# -----------------------------------------------------------------------------
inner_join(caged_rais_preenchido, cnct_cbo, by = c("cbo6" = "Co_Ocupacao")) %>% 
  ...
  select(ANO, MES, CO_UF, NM_UF, Eixo_Tecnologico, Area_Tecnologica, Cod_Curso, 
         ...
         soma_mov_adm, media_sal_des, soma_mov_des, dif_sal_adm_des_pc_m12, tx_rotatividade_m12, estoque_liquido) -> caged_rais_curso 
...
caged_rais_curso <- caged_rais_curso %>% 
  ...
  mutate( estoque_liquido_br = sum(if_else(NM_UF == "Brasil", NA, estoque_liquido), na.rm = TRUE) ) %>% 
  ungroup() %>% relocate(estoque_liquido_br, .after = estoque_liquido) 
...
tabela_ranking_cursos <- caged_rais_curso %>% 
  ...
  mutate( dif_sal_adm_des_pc = if_else(is.infinite(dif_sal_adm_des_pc), NA, dif_sal_adm_des_pc), 
          ...
          tipologia_escassez = case_when( p_dif_sal_adm_des_pc_m12 >= 75 & p_tx_rotatividade_m12 >= 50 & p_estoque_liquido >= 60 ~ "Alerta de Escassez", 
                                          (p_dif_sal_adm_des_pc_m12 >= 75 & p_tx_rotatividade_m12 >= 50 & p_estoque_liquido < 60) | (between(p_dif_sal_adm_des_pc_m12, 74.9999, 50) & p_tx_rotatividade_m12 >= 50 & p_estoque_liquido >= 50) ~ "Tendência de Escassez", 
  ...
  slice_head(n = 10) %>% 
  transmute( `Eixo - Curso` = paste(Eixo_Tecnologico, Curso, sep = " - "), 
...
# -----------------------------------------------------------------------------
obj_rank <- "tabela_ranking_cursos"
assign(obj_rank, tabela_ranking_cursos)

rank_rda_local <- file.path(out_local_dir, "df_ranking_cursos_caged_rais.rda")
save(list = obj_rank, file = rank_rda_local)

# (CSV name was .rda in your snippet; assume CSV intended)
