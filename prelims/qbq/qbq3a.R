...
# Merge 
qbq_ocup_cmento1_ <- qbq_ocup %>%  select(CodCBO,`Ocupação`, `Síntese`, PerfilOcupacional) %>% 
  ...
  select(CodCBO, `Ocupação`, `Síntese`, PerfilOcupacional, desArea, desCampo, desConhecimento) %>% 
...
qbq_ocup_cmento1 <- qbq_ocup %>% 
  select(CodCBO, `Ocupação`, `Síntese`, PerfilOcupacional) %>%
  filter(!is.na(PerfilOcupacional) & !is.null(PerfilOcupacional) & PerfilOcupacional!="NULL") %>%
  left_join(qbq_conhecimento1, by = "CodCBO") %>%
  filter(!is.na(desArea) & !is.null(desArea) & desArea!="NULL") %>%
  distinct() %>%
  group_by(CodCBO, `Ocupação`, `Síntese`, PerfilOcupacional) %>%
  summarise(
