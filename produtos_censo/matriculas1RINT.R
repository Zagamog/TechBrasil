...
geo_vars <- df_codes_ibge %>%
  select(CO_MUN, CO_UF, SG_UF, NM_UF, CO_RGINTM, NM_RGIINTM) %>%
  distinct()
...
df_tudos_aggRGINTM <- map_dfr(anos_validos, function(ano) {
  ...
  df %>%
    ...
    mutate(ANO = ano, AGREG = "RGINTM_TUDO")
})

#Identify columns with only zeros
cols_all_zero <- df_tudos_aggRGINTM %>%
...
df_tudos_aggRGINTM <- df_tudos_aggRGINTM %>%
...
df_red_aggRGINTM <- map_dfr(names(tp_dependencias), function(red_name) {
...
df_pub_aggRGINTM <- df_red_aggRGINTM %>%
...
cols_all_zero <- df_pub_aggRGINTM %>%
  select(any_of(qt_vars_all)) %>%
  summarise(across(everything(), ~ all(. == 0))) %>%
  pivot_longer(everything(), names_to = "var", values_to = "all_zero") %>%
  filter(all_zero) %>%
...
df_pub_aggRGINTM <- df_pub_aggRGINTM %>%
  mutate(across(all_of(cols_all_zero), ~ na_if(., 0)))
...
df_tudos_aggRGINTM <- replace_zero_cols_with_na_by_year(df_tudos_aggRGINTM, qt_vars_all)
df_red_aggRGINTM     <- replace_zero_cols_with_na_by_year(df_red_aggRGINTM, qt_vars_all)
df_pub_aggRGINTM      <- replace_zero_cols_with_na_by_year(df_pub_aggRGINTM, qt_vars_all)


# Combine all aggregated data frames into one
df_censo_RGINTM <- bind_rows(df_tudos_aggRGINTM, df_red_aggRGINTM, df_pub_aggRGINTM)
