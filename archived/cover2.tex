\newcommand{\coverpage}[5]{
    ...
    % #3 - author
    % #4 - date
    % #5 - image path (instead of subject text)
    \pagestyle{empty}
    ...
    %%%%%%%%%%%%%%%%%%%% Image (instead of hexagon with text) %%%%%%%%%%%%%%%%%%%%
    \begin{pgfonlayer}{fg}
    
    % Option 1: Image clipped to hexagon shape (RECOMMENDED)
    \begin{scope}
        \clip ($(current page.west)+(2.5,-5)$) 
              ++(0:2.5cm) -- ++(60:2.5cm) -- ++(120:2.5cm) -- 
              ++(180:2.5cm) -- ++(240:2.5cm) -- ++(300:2.5cm) -- cycle;
        \node[anchor=center] at ($(current page.west)+(2.5,-5)$) {
            \includegraphics[width=6cm,height=6cm,keepaspectratio]{#5}
        };
    \end{scope}
    % Add hexagon border
    \drawhexagon{}{$(current page.west)+(2.5,-5)$}{2.5 cm}{0}{accent!60,draw,line width=2pt};  
