...
ui <- fluidPage(
  div(
    ...
    div(
      ...
      "Brasil: Transição Demográfica por Faixa Etária por Região e Estado"
    )
  ),
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      checkboxInput(
        "showTransition",
        label = "Mostrar Linha de Transição Demográfica (Ponto onde a população em declínio de 0-14 anos cruza com a população crescente de 60+ anos)",
        value = FALSE
      )
    ),
    mainPanel(
      ...
      HTML("<p style='font-size: 12px; color: #555; margin-top: 10px;'>
        ...
        target='_blank'>Projeções Populacionais IBGE</a>
       </p>")
    )
  )
)
...
server <- function(input, output, session) {
  ...
  output$linePlot <- renderPlotly({
    ...
    p <- ggplot(filtered_data, aes(x = ANO, color = LOCAL)) +
      labs(
        x = "Ano",
        y = ifelse(input$dataType == "numbers", "Contagem Populacional", "Proporção"),
