...
df_tudos_aggUF <- map_dfr(anos_validos, function(ano) {
  ...
  df %>%
    ...
    mutate(ANO = ano, AGREG = "UF_TUDO")
})

#Identify columns with only zeros
cols_all_zero <- df_tudos_aggUF %>%
...
df_red_aggUF <- map_dfr(names(tp_dependencias), function(red_name) {
...
df_tudos_aggUF <- replace_zero_cols_with_na_by_year(df_tudos_aggUF, qt_vars_all)
df_red_aggUF     <- replace_zero_cols_with_na_by_year(df_red_aggUF, qt_vars_all)
df_pub_aggUF      <- replace_zero_cols_with_na_by_year(df_pub_aggUF, qt_vars_all)


# Combine all aggregated data frames into one
df_censo_UF <- bind_rows(df_tudos_aggUF, df_red_aggUF, df_pub_aggUF)

save(df_censo_UF, file = "working/mec_inep/df_censo_UF.rda")

# Upload para S3 se necessário
s3_key_final <- "working/mec_inep/df_censo_UF.rda"
tryCatch({
  upload_if_missing_or_changed("working/mec_inep/df_censo_UF.rda", s3_key_final, bucket_name)
}, error = function(e) {
